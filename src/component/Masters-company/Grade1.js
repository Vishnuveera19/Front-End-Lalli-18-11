...
export default function GradeForm1() {
  ...
  // Fetch company data based on logged-in user
  useEffect(() => {
    async function fetchCompanyData() {
      try {
        const companyData = await postRequest(ServerConfig.url, REPORTS, {
          query: `select * from paym_Company where company_user_id = '${user}'`,
        });
  ...
  // Fetch branch data based on selected company
  useEffect(() => {
    ...
    if (pnCompanyId) fetchBranchData();
  }, [pnCompanyId]);
